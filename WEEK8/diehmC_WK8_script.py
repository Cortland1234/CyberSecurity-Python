...
def main():
    ...
    for i in range(1,10000):
        ...
        uniPacket = (ipOBJ.src_address, ipOBJ.dst_address, protocolName)
        
        if uniPacket in captureDict:
            captureDict[uniPacket] += 1
        else:
            captureDict[(ipOBJ.src_address, ipOBJ.dst_address, protocolName)] = 1
        
    tbl = PrettyTable(["Occurs", "SRC", "DST", "Protocol"])
    
    for uniPacket, count in captureDict.items():
        tbl.add_row([count]+list(uniPacket))
